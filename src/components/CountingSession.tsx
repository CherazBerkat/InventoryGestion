...
export default function CountingSessionManager({ 
  ...
  const getItemsForSession = (sessionNumber: 1 | 2 | 3) => {
  ...
  const getSessionProgress = (sessionNumber: 1 | 2 | 3) => {
    const itemsToCount = getItemsForSession(sessionNumber);
    if (!itemsToCount || itemsToCount.length === 0) return { completed: 0, total: 0 };
    
    const countingField = `counting${sessionNumber}` as keyof InventoryItem;
    const completed = itemsToCount.filter(item => item[countingField] !== undefined).length;
    return { completed, total: itemsToCount.length };
  };

  const handleSessionSelect = (sessionNumber: 1 | 2 | 3) => {
  ...
  const isSessionCompleted = (sessionNumber: 1 | 2 | 3) => {
    const progress = getSessionProgress(sessionNumber);
    return progress.completed === progress.total && progress.total > 0;
  };

  const canStartSession = (sessionNumber: 1 | 2 | 3) => {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {[1, 2, 3].map((sessionNum) => {
          const session = sessionNum as 1 | 2 | 3;
      ...
      <div className="flex items-center justify-between p-4 bg-blue-50 rounded-lg border border-blue-200">
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">Session Active: {currentSession.sessionNumber}</span>
          </div>
